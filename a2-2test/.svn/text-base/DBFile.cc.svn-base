...
void DBSorted::Merge (){
  ...
  sprintf(tempFileName, "%s%d", "mergetemp", getppid());
	...
	while (moreFile||morePipe){
		if (moreFile && (!morePipe || comp.Compare(&fRecord, &qRecord, startup->om) <= 0)) {
			//frecord is smaller
...
int DBSorted::GetNext (Record &fetchme,CNF &cnf, Record &literal){
	...
	for (int i = 0; i < SortOrder->numAtts; i++) {
			...
			if ((literalIndex = cnf.hasSimpleEqual(att)) != -1) {
				...
				literalOrderMaker->AddAttribute(literalIndex, SortOrder->GetType(i));
			} else {
				break;
			}
		}
	...
	while (low <= high) {
		...
		page.GetFirst(&fetchme);
		result = comp.Compare(&fetchme, searchOrderMaker, &literal, literalOrderMaker);
		if (result < 0) {
			page.GetFirst(&fetchme);
			while((result = comp.Compare(&fetchme, searchOrderMaker, &literal, literalOrderMaker)) < 0){
				if(page.GetFirst(&fetchme)==0) break;
			}
	...
	while (!comp.Compare(&fetchme, &literal, &cnf)) {
		...
		if (comp.Compare(&fetchme, searchOrderMaker, &literal, literalOrderMaker) != 0) {
			return 0;
		}
		if(page.GetFirst(&fetchme)==0){
				if(pageCounter>=file.GetLength()-2){
