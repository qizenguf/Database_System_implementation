...
#ifdef _WIN32
#define lseek fseek
#define read(file,bits,size) fread(bits,1,size,file)
#define write(file,bits,size) fwrite(bits,1,size,file)
...
void File :: AddPage (Page *addMe, off_t whichPage) {
	//whichPage should start from zero
	if(whichPage < 0){
		cerr << "whichPage Can not smaller than zero\n";
		exit(0);
	}
	// this is because the first page has no data
	whichPage++;
...
void File :: Open (int fileLen, const char *fName) {
	...
	myFilDes = 0;
	if(fileLen == 0){
		myFilDes = fopen(fName,"wb+");
	}else{
		myFilDes = fopen(fName,"rb+");
	}
	if(myFilDes == 0){
