...
void test1 () {
	...
	while (proc && res) {
		...
		if (x < 3) {
			proc = add_data (tblfile,rand()%(int)pow(1e3,x)+(x-1)*1000, res);
			tot += proc;
			if (proc) 
...
int main (int argc, char *argv[]) {
	...
	OrderMaker o;
	rel->get_sort_order (o);
	FileInfo::write(rel->path(), sorted, &o);
	OrderMaker m;
	fType f;
	FileInfo::read(rel->path(), f, m);
	cout << "fileType: " << f << endl;
	if(f != heap){
		cout << "Num of Attr: " << m.numAtts << endl;
		for(int i = 0; i < m.numAtts; i++){
			cout << "Attr" << i <<":"<< m.whichAtts[i]<< endl;
		}
		for(int i = 0; i < m.numAtts; i++){
			cout << "Type" << i <<":"<< m.whichTypes[i]<< endl;
		}
	}
	return 0;*/
