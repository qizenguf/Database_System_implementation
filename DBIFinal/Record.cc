...
int Record :: ComposeRecord (Schema *mySchema, const char *src) {
	...
	if (space == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT1 !!!\n";
		exit(1);
	}
	...
	if (recSpace == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT2 !!!\n";
		exit(1);
	}
	...
	if (bits == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT3 !!!\n";
		exit(1);
	}
...
int Record :: SuckNextRecord (Schema *mySchema, FILE *textFile) {
	...
	if (bits == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT4 !!!\n";
		exit(1);
	}
...
void Record :: CopyBits(char *bits, int b_len) {
	...
	if (this->bits == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT5 !!!\n";
		exit(1);
	}
...
void Record :: Copy (Record *copyMe) {
	...
	if (bits == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT 6!!!\n";
		exit(1);
	}
...
void Record :: Project (int *attsToKeep, int numAttsToKeep, int numAttsNow) {
	...
	if (newBits == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT 7!!!\n";
		exit(1);
	}
...
void Record :: MergeRecords (Record *left, Record *right, int numAttsLeft, int numAttsRight, int *attsToKeep, int numAttsToKeep, int startOfRight) {
	...
	if (bits == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT8 !!!\n";
		exit(1);
	}
