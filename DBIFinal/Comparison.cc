...
int Comparison::IsEqual(int att) {
	if (op == Equals) {
	if (whichAtt1 == att) {
		return whichAtt2;
	} else {
...
OrderMaker::OrderMaker (ifstream& fin){
	...
	for(int i=0; i<numAtts;i++){
		...
		switch (tp)
		{
		case 0: whichTypes[i] = Int; break;
		case 1: whichTypes[i] = Double; break;
		case 2: whichTypes[i] = String; break;
		}
	}
}
...
void OrderMaker::fWrite (ofstream& fout){
	fout << numAtts << endl;

		for (int i = 0; i < numAtts; i++)
		{
			fout << whichAtts[i] << endl;
			fout << whichTypes[i] << endl;
...
Type OrderMaker::GetType(int index){
	return whichTypes[index];
}
int  OrderMaker::GetAttribute(int index){
	return whichAtts[index];

}
void OrderMaker :: setAttsAndTypes(int *atts, Type *types) {
...
void OrderMaker :: copyOrder(OrderMaker *order) {

	numAtts = order->getNumAtts();
	int *Atts = order->getWhichAtts();
	Type *types = order->getWhichTypes();
...
int CNF::hasSimpleEqual(int attribute) {
	int literal;
	for (int i = 0; i < numAnds; i++) {
		if (orLens[i] == 1 && (literal = orList[i][0].IsEqual(attribute)) != -1) {
			return literal;
		}
	}
	return -1;
